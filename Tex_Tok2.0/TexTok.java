...
public class TexTok {
    ...
    private void generarInfoTextosRec(NodoArbolUsuarios actual, ObjectOutputStream out) throws IOException {
        if (actual != null) {
            ...
            // Procesa textos solo si tiene
            NodoTexto texto = actual.getTextosCreados();
            while (texto != null) {
                TextoData tex = new TextoData(
                    actual.getNick(),
                    texto.getFecha(),
                    texto.getTexto()
                );
    ...
    public void cargarTextosDesdeArchivos() {
        ...
        if (!archivo.exists()) {
            System.out.println("No existe archivo de Textos");
            return;
        }
        ...
        try (ObjectInputStream in = new ObjectInputStream(new FileInputStream(archivo))) {
            ...
            while (true) {
                try {
                    TextoData data = (TextoData) in.readObject();

                    // Usuario creador
                    NodoArbolUsuarios usuario = miArbol.buscarUsuario(data.getNick());
                    if (usuario == null) {
                        System.out.println("Usuario NO encontrado para texto: " + data.getNick());
                        continue;
                    }

                    // Crear y agregar texto
                    NodoTexto nuevo = new NodoTexto(data.getTexto(), data.getFecha());

                    if (usuario.getTextosCreados() == null) {
                        usuario.setTextosCreados(nuevo);
                    } else {
                    insertarTexto(nuevo, usuario);
                    }
                    // AGREGAR A LA LISTA GLOBAL DE TEXTOS VISIBLES
                    TextosMasVistos.insertarOrdenado(nuevo);

                    //System.out.println(" Texto cargado para usuario: " + data.getNick());

                } catch (EOFException e) {
                    break;
                } catch (ClassNotFoundException e) {
                    System.out.println(" Error al leer texto: " + e.getMessage());
                    break;
                }
            }

        System.out.println(" Textos cargados correctamente.");

        } catch (IOException e) {
            System.out.println("Error al cargar archivo de textos: " + e.getMessage());
        }
    }


    // ------ Vistas------ //
    public void guardarVistas() {
        File archivo = new File("ValDFVistas.ser");

        try {
            // Siempre rescribimos TODO sin append
            ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream(archivo));

            // Recorremos todo el árbol y guardamos TODAS las vistas en memoria
            guardarVistasRec(miArbol.getRaiz(), out);

            out.close();
            System.out.println("Vistas guardadas con éxito y actualizadas.");

        } catch (IOException e) {
            System.out.println("Error al guardar vistas: " + e.getMessage());
        }
    }


    private void guardarVistasRec(NodoArbolUsuarios actual, ObjectOutputStream out) throws IOException {
        if (actual != null) {

            guardarVistasRec(actual.getAnterior(), out);

            NodoVisto visto = actual.getTextosVisto();
            while (visto != null) {
                NodoTexto texto = visto.getTextoVisto();
                NodoArbolUsuarios creador = miArbol.buscarCreador(texto);

                if (creador != null) {
                    VistaData data = new VistaData(
                            creador.getNick(),
                            texto.getTexto(),
                            actual.getNick()
                    );
                ...
                visto = visto.getVistoSiguiente();
            }

            guardarVistasRec(actual.getSiguiente(), out);
        }
    }

    public void cargarVistasDesdeArchivos() {
        File archivo = new File("ValDFVistas.ser");

        if (!archivo.exists()) {
            System.out.println("No existe archivo de Vistas");
            return;
        }
        ...
        while (true) {
                try {
                    VistaData vista = (VistaData) in.readObject();

                    // Autor del texto
                    NodoArbolUsuarios autor = miArbol.buscarUsuario(vista.getNickCreador());
                    if (autor == null) continue;

                    // Buscar el texto en la lista del autor
                    NodoTexto texto = buscarTextoPorContenidoRec(autor.getTextosCreados(), vista.getTexto());
            ...
            System.out.println("Vistas cargadas con éxito.");
        }
        catch (IOException e) {
            System.out.println("Error al leer archivo de vistas: " + e.getMessage());
        }   
    }


    private NodoTexto buscarTextoPorContenidoRec(NodoTexto actual, String contenido) {
        NodoTexto encontrado = null;

        if (actual != null) {
            if (actual.getTexto().equals(contenido)) {
                encontrado = actual;
            } else {
                encontrado = buscarTextoPorContenidoRec(actual.getSiguiente(), contenido);
            }
        }

        return encontrado;
    }
    private void insertarVistoRec(NodoVisto actual, NodoVisto nuevo) {
        if (actual.getVistoSiguiente() != null) {
            insertarVistoRec(actual.getVistoSiguiente(), nuevo);
        } else {
